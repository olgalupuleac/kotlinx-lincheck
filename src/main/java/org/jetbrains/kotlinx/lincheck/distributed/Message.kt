...
class Message(val bytes: ByteArray) {
    override fun toString(): String = parse {
        generateSequence {
            when(cur) {
                MessageFieldType.END -> null
                MessageFieldType.INT -> readInt().toString()
                MessageFieldType.STR -> readString()
                MessageFieldType.ENUM -> readEnumName()
            }
...
class MessageBuilder {
    ...
    fun writeInt(value: Int) {
        writeField(MessageFieldType.INT)
        data.writeInt(value)
    }

    fun writeString(str: String) {
        writeField(MessageFieldType.STR)
        data.writeUTF(str)
    }

    fun writeEnum(enum: Enum<*>) {
        writeField(MessageFieldType.ENUM)
...
class MessageParser(message: Message) {
    ...
    fun readInt(): Int {
        check(cur == MessageFieldType.INT) { "Expected int field, but $cur found" }
        return data.readInt().also { done() }
    }

    fun readString(): String {
        check(cur == MessageFieldType.STR) { "Expected string field, but $cur found" }
        return data.readUTF().also { done() }
    }

    fun readEnumName(): String {
        check(cur == MessageFieldType.ENUM) { "Expected enum field, but $cur found" }
        return data.readUTF().also { done() }
    }
    ...
    private fun done() { curField = null }
}
